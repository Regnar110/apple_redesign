...
const Success = ({products}: Props) => {
    ...
    //useRouter daje nam dosep do parametrów dostepnychprze nextJs router
    // parametry poniżej tp np. /succes/session_id <<< mamy dosęp do tego
    ...
    // powyższe sprawdza czy mediaquery jest True. Jeżeli tak wprowadza się na bazie
    // zwrotu z tego mediaquery zmiany w view layout 
    const showOrderSummaryCondition = isTabletOrMobile ? showOrderSummary : true;
    // Warunek, który będzie służył do decydowania o tym czym
    // mamy pokazać podsumowanie zamówienia. Jeżeli jest to na tableciie lub telfonie
    // to pokazujemy, jezeli nie to zwracamy true czyli nie
      ...
      <section className="order-2 mx-auto max-w-xl pb-12 lg:col-span-5 lg:mx-0 lg:max-w-none lg:pr-16 lg:pt-16 xl:pl-16 2xl:pl-44">
        ...
        <div className="my-8 ml-4 flex space-x-4 lg:ml-14 xl:ml-0">
          ...
          <div>
            <p className="text-sm text-gray-600">
              ...
              {/* Tutaj zwracamy tylko 5 ostatnich cyfr query parametru session id.
                ma być to numer zamówienia */}
        ...
        <div className="mx-4 divide-y divide-gray-300 rounded-md border border-gray-300 p-4 lg:ml-14">
          <div className="space-y-2 pb-3">
            ...
            <p className="text-sm text-gray-600">
              We accepted your order, and we are getting it ready. Come back
        ...
        <div className="mx-4 flex flex-col items-center justify-between text-sm lg:ml-14 lg:flex-row">
          ...
          {mounted && (
            // mounted jest po to żeby mediaquery z react-responsive działał
            // biblioteka ta musi wiedzieć że komponent jest już zamontowany
            <Button
              title="Continue Shopping"
              onClick={() => router.push("/")}
              width={isTabletOrMobile ? "w-full" : undefined}
              // jeżeli media query isTabletOrMobile jest true to w-full
              // a jak nie to undefined czyli dopasowaane do contentu
              padding="py-4"
            />
          )}
        </div>
